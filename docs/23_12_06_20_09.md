...
\[
\begin{array}{c}
h(y)=\min\{c^{\top}x\colon Ax\geq b-g(y)\}=\\
...
that $(b-By^{\star})^{\top}v_{j}> 0$. We add a new constraint
$(b-By)^{\top}v_{j}\leq 0$ to problem (4).  

If problem (5) has an optimal solution $u_{i}$, then we have two cases:

1. If $h^{\star}=(b-By^{\star})^{\top}u_{i}$, then $(x^{\star},y^{\star})$ is an 
optimal solution of problem (1) where 
$x^{\star}=\mathrm{argmin}\{c^{\top}x\colon Ax\geq b-g(y^{\star}), x\geq 0\}$. 

2. If $h^{\star}<(b-By^{\star})^{\top}u_{i}$, then we add 
a new constraint $(b-By)^{\top}u_{i}\leq h$ to problem (4).
